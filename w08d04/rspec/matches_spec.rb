...
describe "show the depths of rspec" do
  ...
  context "work through the different types of Rspec matchers" do
    ...
    it 'should test that equality matchers are different' do
      ...
      expect(b).to eql(a) # this will fail as c (float) is not an equivalent object to b (int)
    ...
    it 'should test the various different type of comparison matcher' do
      ...
      it 'should test the various class type matchers' do
        g = 5.0
        h = Hash.new
        # or can do expect(g).to be_instance_of(useful for class) or be_kind_of(Float or Hash)
        expect(g).to be_within(0.0).of(g) #check if g is a float
        expect(h).to include(h) #check if h is a hash
      end
